Feature: PAGATEIAN API TESTING
  ...
  @RegressionSuite @English @APITesting @PassingExternalPayload
  Scenario Outline: Automating End to End Flow
    Given User Logins to the Application
    When I Make An Enquiry with the following details
      | domisili        | 1                         |
    ...
    And I Also Inquire for the Payments with the help of below details
      | terminalid | "<Terminal_ID>" |
    Then I Make the Payment using the given transaction details using the external payload
      | "<payload>" |

    Examples:
      | Terminal_ID | payload              | id_transaksi      | payment | flag |
      | 1234567890  | PaymentsPayload.json | 17476224858570110 | VA_BRI  | K    |
